...
UPDATE coffee_sales
SET transaction_date = STR_TO_DATE(transaction_date, '%m/%d/%Y'); -- Updating the dates in correct format

ALTER TABLE coffee_sales
MODIFY COLUMN transaction_date DATE; -- Updating the datatype of the column consisting dates

UPDATE coffee_sales
SET transaction_time=STR_TO_DATE(transaction_time,'%H:%i:%s'); -- Updating the time in correct format
...
/* TOTAL QUANTITY SOLD AND QUANTITY SOLD IN EACH MONTH*/
